...
class UserDAO {
     ...
     * @returns A promise that resolves to a list of all users in the database
     */
        ...
        return new Promise<User[]>((resolve, reject) => {
            try {
                ...
                db.all(sql, (err: Error | null, rows: any) => {
                    ...
                    const users = rows.map((u: any) => new User(u.username, u.name, u.surname, u.role, u.address, u.birthdate))
                    resolve(users)
                })
            }
            catch(error) {
    ...
    /**
     * Gets all users from the database with the specified role
     * @returns A promise that resolves to a list of users with the specified role
     */
    ...
    deleteUser(username: String): Promise<Boolean> {
        return new Promise<Boolean>((resolve, reject) => {
            try {
                var sql = "SELECT role FROM users WHERE username=?"
                db.get(sql, username, (err: Error | null, row: any) => {
                    if(err) {
                        reject(err)
                        return
                    ...
                    db.all(sql, username, (err: Error | null, row: any) => {
                        if(err) {
                            reject(err)
                            return
                        }
        ...
        return new Promise<User>((resolve, reject) => {
            try {
                ...
                db.get(sql, [username], (err: Error | null, row: any) => {
                    ...
                    db.all(sql, [name, surname, address, birthdate, username], (err: Error | null, row: any) => {
                        if(err) {
                            reject(err)
                            return
                        }
                        ...
                        sql = "SELECT * FROM users WHERE username = ?"      //Get the information of the user just updated
                        db.get(sql, [username], (err: Error | null, row: any) => {
                            if (err) {
                                reject(err)
                                return
