...
class Authenticator {
    ...
    isAdminOrCustomer(req: any, res: any, next: any) {
        if (req.isAuthenticated() && (Utility.isAdmin(req.user) || Utility.isCustomer(req.user))) return next()
            return res.status(401).json({ error: "User is not an admin or customer", status: 401 })
    }
