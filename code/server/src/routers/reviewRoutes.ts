...
class ReviewRoutes {
    ...
    initRoutes() {
        ...
        this.router.post(
            "/:model",
            [param('model').notEmpty().withMessage('Errore: model non può essere vuoto'),
            body('comment').notEmpty().withMessage('Errore: comment non può essere vuoto'),
            body('score').isInt({ min: 1, max: 5 }).withMessage('Errore: score deve essere un intero compreso tra 1 e 5')],
        ...
        this.router.get(
            "/:model",
            [body('model').notEmpty().withMessage('Errore: model non può essere vuoto')],
            (req: any, res: any, next: any) => this.authenticator.isLoggedIn(req, res, next),
