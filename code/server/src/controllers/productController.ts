...
class ProductController {
    ...
    async changeProductQuantity(model: string, newQuantity: number, changeDate: string | null): Promise<number> { 
        ...
        return new Promise<number>((resolve, reject) => {
            (async () => {
                ...
                else if (changeDate && changeDate > today) {
                    reject(new DateError());
                    return
                }
                try {
                  ...
                  await this.dao.increaseQuantity(model, newQuantity);
        ...
        // Vecchia versione
        //return this.dao.increaseQuantity(model, newQuantity);
    ...
    async sellProduct(model: string, quantity: number, sellingDate: string | null)/*: Promise<number>*/ { 
        ...
        return new Promise<number>((resolve, reject) => {
            (async () => {
                ...
                else if (sellingDate && sellingDate > today) {
                    reject(new DateError());
                    return
                }
                try {
                  ...
                  await this.dao.increaseQuantity(model, -quantity);
    ...
    async getAvailableProducts(grouping: string | null, category: string | null, model: string | null): Promise<Product[]> { 
        ...
        if(model){
            const productModel = await this.dao.getProduct(model);
            if(!productModel) throw (new UncorrectGroupingCategoryModelFilter());
        }
        const getAvailableProducts = await this.dao.getAvailableProducts(grouping, category, model);
